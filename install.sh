...
#
#   curl -fsSL https://raw.githubusercontent.com/alinshaikh/insight-orchestra/main/install.sh | bash
...
REPO_URL="https://github.com/alinshaikh/insight-orchestra.git"
...
if [ -d "$INSTALL_DIR" ]; then
  if [ -d "$INSTALL_DIR/.git" ] && git -C "$INSTALL_DIR" remote get-url origin 2>/dev/null | grep -q "alinshaikh/insight-orchestra"; then
